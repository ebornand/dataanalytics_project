...
coordinates_kiew <- c(df_geocode_kiew$locations[[1]]$referencePosition$longitude, df_geocode_kiew$locations[[1]]$referencePosition$latitude)
...
for (i in 1:nrow(countries_capitals_and_more)) {
  ...
  url <- capture.output(cat("https://api.myptv.com/geocoding/v1/locations/by-text?searchText=",
                            gsub(" ", "%", country$country, fixed = TRUE),
                            "%",
                            gsub(" ", "%", country$capital, fixed = TRUE),
  ...
  coordinates_capital <- c(geocode_country$locations[[1]]$referencePosition$longitude, geocode_country$locations[[1]]$referencePosition$latitude)
  print(coordinates_capital)
  distance <- distm(c(as.numeric(coordinates_kiew[1]), as.numeric(coordinates_kiew[2])), c(as.numeric(coordinates_capital[1]), as.numeric(coordinates_capital[2])), fun = distGeo)
